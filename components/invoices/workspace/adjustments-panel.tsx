...
export function AdjustmentsPanel({
  ...
  // Nothing to show and nothing being edited: this is a single link. A titled
  // card whose whole content is "there is nothing here" costs a module's worth
  // of height to say so.
  const isEmpty = !isLoading && adjustments.length === 0 && !formOpen

  return (
    <section
      className={isEmpty ? "" : "rounded-[10px] border border-stone-200 bg-white p-4"}
    >
      {!isEmpty && (
        <div className="flex items-center justify-between">
          <span className="text-[12px] font-extrabold uppercase tracking-[0.05em] text-stone-400">Adjustments</span>
          ...
          {adjustments.map(a => (
            <div key={a.id} className="flex items-center gap-2 rounded-md border border-stone-100 px-2.5 py-2">
              <span className="min-w-0 flex-1">
                <span className="block truncate text-[15px] font-semibold text-stone-800">{a.label}</span>
                <span className="block text-[12.5px] text-stone-400">
                  {MODE_LABELS[a.mode as AdjustmentMode] ?? a.mode}
                  {a.serviceDay ? ` · day ${a.serviceDay}` : ""}
                </span>
              </span>
              <span className={`flex-none text-[15px] font-bold tabular-nums ${a.amount < 0 ? "text-[#15793f]" : "text-stone-800"}`}>
              ...
              <button
                ...
                className={`flex-none rounded-md px-2 py-1 text-[12.5px] font-bold transition-colors ${
                  a.approved
      ...
      {blockedReason && (
        <p className="mt-2 text-[12.5px] font-semibold text-amber-700">{blockedReason}</p>
      )}
      ...
      {formOpen ? (
        ...
        >
          <div className="mb-[11px] text-[16px] font-bold" style={{ color: "#3b4380" }}>
          ...
          <div className="flex flex-wrap gap-[7px]">
            {ADJUSTMENT_MODES.map(m => (
              <button
                ...
                className="rounded-[20px] px-3 py-1.5 text-[15px] font-bold transition-colors"
                style={
                  mode === m
              ...
              <div className="flex gap-1.5">
                {PCT_PRESETS.map(p => (
                  <button
                    ...
                    type="button"
                    onClick={() => setPct(p)}
                    className={`rounded-full border px-3 py-1 text-[15px] font-bold transition-colors ${
          ...
          {MODES_WITH_DAY.includes(mode) && (
            <input
              ...
              className="mt-2.5 w-full rounded-md border border-stone-300 px-2.5 py-1.5 text-[15px] outline-none focus:border-indigo-500"
            />
          )}
          ...
          <input
            ...
            className="mt-2 w-full rounded-md border border-stone-300 px-2.5 py-1.5 text-[15px] outline-none focus:border-indigo-500"
          />
          ...
          <div className="mt-2.5 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5">
              <button type="button" onClick={reset} className="rounded-md px-2.5 py-1.5 text-[15px] font-bold text-stone-500 hover:bg-stone-200">
                Cancel
              </button>
              <button
                type="button"
                onClick={apply}
                disabled={busy}
                className="inline-flex items-center gap-1.5 rounded-[9px] px-4 py-[9px] text-[15px] font-bold text-white transition-opacity hover:opacity-95 disabled:opacity-50"
        ...
        <button
          type="button"
          onClick={openForm}
          className={`inline-flex items-center gap-[7px] text-[15px] font-bold transition-opacity hover:opacity-80 ${isEmpty ? "" : "mt-2.5"}`}
          style={{ color: "#4f46e5" }}
        >
